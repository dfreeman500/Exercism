...
def categorize_dish(dish_name, dish_ingredients):
    ...
    """
    dishes = [VEGAN, VEGETARIAN, PALEO, KETO, OMNIVORE]
    dish_strings = ["VEGAN", "VEGETARIAN", "PALEO", "KETO", "OMNIVORE"]
    for key, value in (zip(dishes, dish_strings)):
        if  (set(dish_ingredients).issubset(key)):
            return f"{dish_name}: {value}"
...
def compile_ingredients(dishes):
    ...
    return set([item for sublist in dishes for item in sublist])
...
def singleton_ingredients(dishes, intersection):
    ...
    # print(intersection)
    return  set([item for sublist in dishes for item in sublist]) - intersection
